...
public class FileUtil extends cn.hutool.core.io.FileUtil {
    ...
    public static FileDto uploadFile(MultipartFile multipartFile) {
        //创建阿里云客户端对象
        OSSClient ossClient = new OSSClient(DataConfig.getEndPoint(), DataConfig.getKeyId(), DataConfig.getKeySecret());
        //设置读写权限
        ossClient.setBucketAcl(DataConfig.getBucket(), CannedAccessControlList.PublicRead);
        ...
        try {
            ...
            if (file.exists()) {
                ...
                //上传文件
                PutObjectResult putObjectResult = ossClient.putObject(DataConfig.getBucket(),
                        DataConfig.getFileDir() + getFileDir(fileType) + fileNewName, inputStream, objectMetadata);
                //判断结果，并将文件信息返回
                if (StringUtil.isNotEmpty(putObjectResult.getETag())) {
                    ...
                    fileEntity.setFilePath(DataConfig.getImageUrl() + fileNewName);
                    return fileEntity;
                } else {
                    return null;
                }
    ...
    public static String getFileUrl(String fileName) {
        ...
        String url = ossClient.generatePresignedUrl(DataConfig.getBucket(),DataConfig.getFileDir() + getFileDir(fileType),
                expiration).toString();
        return url;
    }
